...
=== "Docker"
    ...
    2. Ainda no mesmo diretório, crie mais um arquivo chamado `init-db.js`, e dentro dele adicione o seguinte conteúdo:
    ...
    4. O console do Unifi deve demorar aproximadamente 1 minuto para iniciar.
...
=== "Docker (servidores antigos)"
    ...
    2. Dentro deste novo diretório `unifi`, crie outro chamado `data` e dentro dele adicione um arquivo chamado `init-mongo.sh`:
    ...
    3. Execute o comando: `docker compose up -d` (os logs da execução podem ser acessados com `docker compose logs`)
    4. A interface web da controladora Unifi deve demorar aproximadamente 1 minuto para iniciar.
    5. Acesse o console via `https://ip_da_maquina:8443`
